...
\begin{itemize}
    \itembox Pushing doubles Infection Rating. User also automatically suffers \dv{1d6}.
...
\begin{itemize}
   \itembox Modified behavior: If IR $<$ 33 player is compelled to perform or avoid. If IR $<$ 66 a \skill{WIL} check is needed. If IR $\geq$ 66 also suffer \dice{1d6} stress if not complying.
   \itembox If motivation played well, recover stress. If not fulfilled, receive \dice{1d6} stress. Resisting motivation requires \skill{WIL} and inflicts \modifier{-10} to all actions.
\end{itemize}
